...
class apiREST
{
    ...
    static public function execute (apiServiceRequest $req)
    {
        ...
        $url = self::createRequestUri($req->getRestBasePath(),
                $req->getRestPath(), $req->getParameters());

        $httpRequest = new apiHttpRequest($url, $req->getHttpMethod(), null,
                $postBody);
        if ($postBody) {
            ...
            if (isset($req->contentType) && $req->contentType) {
                ...
                $contentTypeHeader['content-length'] = apiUtils::getStrLen(
                        $postBody);
            }
        ...
        // hopefully this might change some day
    ...
    static function decodeHttpResponse ($response)
    {
        ...
        if ($code != '200' && $code != '201' && $code != '204') {
            ...
            $err = 'Error calling ' . $response->getRequestMethod() . ' ' .
                    $response->getUrl();
            if ($decoded != null &&
                    isset($decoded['error']['message']) &&
                    isset($decoded['error']['code'])) {
                // if we're getting a json encoded error
                // definition, use that
        ...
        if ($code != '204') {
            ...
            if ($decoded == null) {
                throw new apiServiceException(
                        "Invalid json in service response: $body");
            }
        }
        return $decoded;
    }
    ...
    static function createRequestUri ($basePath, $restPath, $params)
    {
        ...
        foreach ($params as $paramName => $paramSpec) {
            // Discovery v1.0 puts the canonical location under the
            // 'location'
            // field.
        ...
        if (count($uriTemplateVars)) {
            $uriTemplateParser = new URI_Template_Parser(
                    $requestUrl);
            $requestUrl = $uriTemplateParser->expand(
                    $uriTemplateVars);
        }
        // FIXME work around for the the uri template lib which
        // url encodes
