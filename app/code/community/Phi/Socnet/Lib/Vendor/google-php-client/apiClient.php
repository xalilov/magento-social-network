...
// Check for the required json and curl extensions, the Google API PHP Client
...
class apiClient
{
    ...
    public function discover ($service, $version = 'v1')
    {
        ...
        $this->$service = $this->discoverService($service,
                $this->services[$service]['discoveryURI']);
        return $this->$service;
    }
    ...
    public function addService ($service, $version)
    {
        ...
        if ($this->authenticated) {
            // Adding services after being authenticated, since the oauth scope
            // is already set (so you wouldn't have access to that data)
        ...
        if (isset($apiConfig['services'][$service])) {
            ...
            $this->services[$service] = array_merge($this->services[$service],
                    $apiConfig['services'][$service]);
        }
        $this->services[$service]['discoveryURI'] = $apiConfig['basePath'] .
        '/discovery/' . self::discoveryVersion . '/describe/' .
        urlencode($service) . '/' . urlencode($version);
    }
     ...
     *
     * @param array $scopes,
     *            ie: array('https://www.googleapis.com/auth/plus',
    ...
    private function discoverService ($serviceName, $serviceURI)
    {
        ...
        if ($request->getResponseHttpCode() != 200) {
            throw new apiException(
                    "Could not fetch discovery document for $serviceName, code: " .
                    $request->getResponseHttpCode() . ", response: " .
