...
class apiModel
{
     ...
     *
     * @param
     *            array Used to seed this object's properties.
     * @return void
     */
                ...
                if ($this->isAssociativeArray($val)) {
                    if (isset($this->$keyDataType) &&
                            ...
                            foreach ($val as $arrayKey => $arrayItem) {
                                $val[$arrayKey] = $this->createObjectFromName(
                                        $keyTypeName, $arrayItem);
                            }
                            $this->$key = $val;
                    } else {
                        $this->$key = $this->createObjectFromName(
                                $keyTypeName, $val);
                    }
                } else
                    if (is_array($val)) {
                    ...
                    foreach ($val as $arrayIndex => $arrayItem) {
                        $arrayObject[$arrayIndex] = $this->createObjectFromName(
                                $keyTypeName, $arrayItem);
                    }
    ...
    protected function assertIsArray ($obj, $type, $method)
    {
        if ($obj && ! is_array($obj)) {
            throw new apiException(
                    "Incorrect parameter type passed to $method(), expected an" .
