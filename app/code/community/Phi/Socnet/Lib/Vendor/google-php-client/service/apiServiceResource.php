 ...
 * is available in this service, and if so construct an apiServiceRequest
 * representing it.
 *
...
class apiServiceResource
{
    ...
    public function __call ($name, $arguments)
    {
        if (count($arguments) != 1 && count($arguments) != 2) {
            throw new apiException(
                    "client method calls expect 1 or 2 parameter (\$client->plus->activities->list(array('userId' => 'me'))");
        }
        if (! is_array($arguments[0])) {
            throw new apiException(
                    "client method parameter should be an array (\$client->plus->activities->list(array('userId' => 'me'))");
        }
        $batchKey = false;
        if (isset($arguments[1])) {
            if (! is_string($arguments[1])) {
                throw new apiException(
                        "The batch key parameter should be a string (\$client->plus->activities->list( array('userId' => 'me'), 'batchKey'))");
            }
            $batchKey = $arguments[1];
        }
        if (! isset($this->methods[$name])) {
            throw new apiException(
                    "Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
        }
        ...
        if (isset($parameters['postBody'])) {
            ...
            is_object($parameters['postBody']) ? json_encode(
                    $parameters['postBody']) : $parameters['postBody'];
            unset($parameters['postBody']);
        ...
        // Discovery v1.0 puts the canonical method id
        // under the 'id' field.
        if (! isset($method['id'])) {
            $method['id'] = $method['rpcMethod'];
        }

        // Discovery v1.0 puts the canonical path under
        // the 'path' field.
        ...
        if (isset($method['mediaUpload'])) {
            $media = apiMediaFileUpload::process(
        ...
        $request = new apiServiceRequest($restBasePath,
                $this->service->rpcPath,
                $method['path'], $method['id'],
                $method['httpMethod'], $parameters,
                $postBody);
    ...
    {
        global $apiConfig;
        return (isset($apiConfig['use_objects']) &&
                $apiConfig['use_objects']);
    }
