...
class apiCurlIO implements apiIO
{
    ...
    private static $HOP_BY_HOP = array('connection', 'keep-alive',
            'proxy-authenticate', 'proxy-authorization', 'te', 'trailers',
            'transfer-encoding', 'upgrade');

    private static $DEFAULT_CURL_PARAMS = array(CURLOPT_RETURNTRANSFER => true,
            CURLOPT_FOLLOWLOCATION => 0, CURLOPT_FAILONERROR => false,
            CURLOPT_SSL_VERIFYPEER => true, CURLOPT_HEADER => true);
    ...
    public function makeRequest (apiHttpRequest $request)
    {
        ...
        if ($cached !== false) {
            if (apiCacheParser::mustRevalidate($cached)) {
                ...
                if ($cached->getResponseHeader('etag')) {
                    // [13.3.4] If an entity tag has been provided by the origin
                    // server,
                    // we must use that entity tag in any cache-conditional
                    // request.
                    $addHeaders['If-None-Match'] = $cached->getResponseHeader(
                            'etag');
                } elseif ($cached->getResponseHeader('date')) {
                    $addHeaders['If-Modified-Since'] = $cached->getResponseHeader(
                            'date');
                }
        ...
        list ($responseHeaders, $responseBody) = $this->parseHttpResponseBody(
                $respData, $respHeaderSize);

        if ($respHttpCode == 304 && $cached) {
            // If the server responded NOT_MODIFIED, return the cached request.
            if (isset($responseHeaders['connection'])) {
                $hopByHop = array_merge(self::$HOP_BY_HOP,
     ...
     *
     * @param apiHttpRequest $request
     * @return apiHttpRequest bool the cached object or
    ...
    public function parseHttpResponseBody ($respData, $headerSize)
    {
        if (stripos($respData, self::CONNECTION_ESTABLISHED) !== false) {
            $respData = str_ireplace(self::CONNECTION_ESTABLISHED, '',
                    $respData);
        }
    ...
    public function processEntityRequest (apiHttpRequest $request)
    {
        ...
        if (! $postBody || is_string($postBody)) {
            ...
            $request->setRequestHeaders(
                    array('content-length' => $postsLength));
        }
