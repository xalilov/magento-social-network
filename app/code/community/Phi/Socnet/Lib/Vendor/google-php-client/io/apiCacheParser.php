...
class apiCacheParser
{
    ...
    public static function isResponseCacheable (apiHttpRequest $resp)
    {
        ...
        // [rfc2616-14.9.2] If [no-store is] sent in a response, a cache MUST
        // NOT
        ...
        // [rfc2616-14.44] Vary: * is extremely difficult to cache. "It implies
        // that
    ...
    public static function isExpired (apiHttpRequest $resp)
    {
        ...
        if (isset($responseHeaders['expires'])) {
            ...
            if (empty($rawExpires) ||
                    (is_numeric($rawExpires) && $rawExpires <= 0)) {
                return true;
            }
    ...
    /**
     * Determine if a cache entry should be revalidated with by
     * the origin.
     *
     * @param apiHttpRequest $response
     * @return bool True if the entry is expired, else return
     *         false.
     */
    public static function mustRevalidate (
            apiHttpRequest $response)
    {
        // [13.3] When a cache has a stale entry that it would
        // like to use as a
        // response to a client's request, it first has to check
        // with the origin
