...
/**
 * Authentication class that deals with the OAuth 2 web-server authentication
 * flow
 *
...
class apiOAuth2 extends apiAuth
{
     ...
     * to the discretion of the caller (which is done by calling
     * authenticate()).
     */
    ...
    public function authenticate ($service)
    {
        if (isset($_GET['code'])) {
            ...
            $request = apiClient::$io->makeRequest(
                    new apiHttpRequest(self::OAUTH2_TOKEN_URI, 'POST', array(),
            ...
            if ($request->getResponseHttpCode() == 200) {
                ...
                if ($decodedResponse != $response && $decodedResponse != null &&
     ...
     *
     * @param string $scope
     *            The scope is expressed as a list of space-delimited
     *            strings.
     * @return string
     */
    ...
    public function sign (apiHttpRequest $request)
    {
        ...
        // If the token is set to expire in the next 30 seconds (or has
        // already
        // expired), refresh it and set the new token.
        $expired = ($this->accessToken['created'] +
                ($this->accessToken['expires_in'] - 30)) < time();
        if ($expired) {
    ...
    public function refreshToken ($refreshToken)
    {
        ...
        $request = apiClient::$io->makeRequest(
                new apiHttpRequest(self::OAUTH2_TOKEN_URI, 'POST',
                        array(), $params));
        ...
        if ($code == 200) {
            ...
            throw new apiAuthException(
                    "Error refreshing the OAuth2 token, message: '$body'",
                    $code);
        }
    }
     ...
     *            The token (access token or a refresh token)
     *            that should be
     *            revoked.
     * @return boolean Returns True if the revocation was
    ...
    public function revokeToken ($token = null)
    {
        ...
        $request = new apiHttpRequest(self::OAUTH2_REVOKE_URI,
    ...
    // Gets federated sign-on certificates to use for verifying
    // identity tokens.
    // Returns certs as array structure, where keys are key ids,
    // and values
    // are PEM encoded certificates.
    private function getFederatedSignOnCerts ()
    {
        // This relies on makeRequest caching certificate
        // responses.
        $request = apiClient::$io->makeRequest(
                new apiHttpRequest(
                        self::OAUTH2_FEDERATED_SIGNON_CERTS_URL));
        if ($request->getResponseHttpCode() == 200) {
            $certs = json_decode($request->getResponseBody(),
                    true);
            if ($certs) {
                return $certs;
            }
    ...
    /**
     * Verifies an id token and returns the authenticated
     * apiLoginTicket.
     * Throws an exception if the id token is not valid.
     * The audience parameter can be used to control which id
     * tokens are
     * accepted. By default, the id token must have been issued
     * to this OAuth2
     * client.
     *
     * @param
     ...
     */
    public function verifyIdToken ($id_token = null,
            $audience = null)
    {
        if (! $id_token) {
        ...
        return $this->verifySignedJwtWithCerts($id_token,
                $certs, $audience);
    }

    // Verifies the id token, returns the verified token
    // contents.
    // Visible for testing.
            ...
            $required_audience)
    {
        ...
        $envelope = json_decode(
                apiUtils::urlSafeB64Decode($segments[0]), true);
        if (! $envelope) {
            throw new apiAuthException(
