...
/**
 * This class implements the experimental JSON-RPC transport for executing
 * apiServiceRequest'
 *
...
class apiRPC
{
    ...
    static public function execute ($requests)
    {
        ...
        foreach ($requests as $request) {
            ...
            $jsonRpcRequest[] = array('id' => $request->getBatchKey(),
                    'method' => $request->getRpcName(), 'params' => $parameters,
                    'apiVersion' => 'v1');
        }
        ...
        if (($decodedResponse = json_decode($httpRequest->getResponseBody(),
                true)) != false) {
            $ret = array();
            foreach ($decodedResponse as $response) {
                $ret[$response['id']] = self::checkNextLink(
                        $response['result']);
            }
            return $ret;
        } else {
    ...
    static private function checkNextLink ($response)
    {
        if (isset($response['links']) &&
                isset($response['links']['next'][0]['href'])) {
