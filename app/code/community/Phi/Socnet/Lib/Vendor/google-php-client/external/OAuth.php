...
class apiClientOAuthToken
{
    ...
    {
        return "oauth_token=" .
                apiClientOAuthUtil::urlencodeRFC3986($this->key) .
                "&oauth_token_secret=" .
                apiClientOAuthUtil::urlencodeRFC3986($this->secret);
    }
...
class apiClientOAuthSignatureMethod_HMAC_SHA1 extends apiClientOAuthSignatureMethod
{
            ...
            $privKey = NULL)
    {
        ...
        $key_parts = array_map(
                array('apiClientOAuthUtil', 'urlencodeRFC3986'), $key_parts);
...
class apiClientOAuthSignatureMethod_RSA_SHA1 extends apiClientOAuthSignatureMethod
{
    ...
    { // &$request) {
        ...
        // ideas are:
        // (1) do a lookup in a
        // table of trusted certs
        // keyed off of consumer
        //
        // either way should
        // return
...
class apiClientOAuthRequest
{
            ...
            $parameters = NULL)
    {
        ...
        // let the library user override things however they'd like, if they
        // know
        // which parameters to use then go for it, for example XMLRPC might want
        // to
        // do this
        if ($parameters) {
            $req = new apiClientOAuthRequest($http_method, $http_url,
                    $parameters);
        ...
        // so that we can include them in the signature
            ...
            $header_parameters = apiClientOAuthRequest::split_header(
    ...
    public function get_signable_parameters ()
    {
        ...
        $keys = array_map(
                array('apiClientOAuthUtil', 'urlencodeRFC3986'),
                array_keys($params));
        $values = array_map(
                array('apiClientOAuthUtil', 'urlencodeRFC3986'),
                array_values($params));
    ...
    public function get_signature_base_string ()
    {
        ...
        $parts = array_map(
                array('apiClientOAuthUtil', 'urlencodeRFC3986'), $parts);
    ...
    public function to_postdata ()
    {
        ...
        foreach ($this->parameters as $k => $v) {
            $total[] = apiClientOAuthUtil::urlencodeRFC3986($k) . "=" .
                    apiClientOAuthUtil::urlencodeRFC3986($v);
        }
        $out = implode("&", $total);
        return $out;
    }
    ...
    public function to_header ()
    {
        ...
        foreach ($this->parameters as $k => $v) {
            if (substr($k, 0, 5) != "oauth")
                continue;
            $out .= apiClientOAuthUtil::urlencodeRFC3986($k) .
            '="' .
            apiClientOAuthUtil::urlencodeRFC3986($v) .
            '", ';
        }
            ...
            $token, $privKey = NULL)
    {
        ...
        $signature = $this->build_signature($signature_method,
                $consumer, $token, $privKey);
        $this->set_parameter("oauth_signature", $signature);
    }

    public function build_signature ($signature_method,
            $consumer, $token, $privKey = NULL)
    {
    ...
    private static function generate_nonce ()
    {
        ...
        return md5($mt . $rand); // md5s look nicer than
        // numbers
    }
    ...
    private static function split_header ($header)
    {
        ...
        foreach ($parts as $param) {
            $param = ltrim($param);
            // skip the "realm" param, nobody ever uses it
            // anyway
            ...
            // rawurldecode() used because urldecode() will turn
            // a "+" in the
            // value into a space
            $out[$param_parts[0]] = rawurldecode(
                    substr($param_parts[1], 1, - 1));
        }
        return $out;
    }

    /**
     * helper to try to sort out headers for people who aren't
     * running apache
     */
...
class apiClientOAuthDataStore
{
    ...
    function fetch_access_token ($token, $consumer)
    {
        ...
        // for the user associated with this token if the
        // request token
        // is authorized
